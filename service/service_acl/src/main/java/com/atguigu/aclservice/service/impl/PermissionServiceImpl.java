...
//        });
    ...
    public List<Permission> queryAllMenuGuli2() {
        // 先初始化一个list,用于存储整理好的三级分类树状结构
        List<Permission> permissions = new ArrayList<Permission>();
        // 思路：首先获取所有的list，然后找到入口：即一级分类
        QueryWrapper<Permission> permissionQueryWrapper = new QueryWrapper<Permission>();
        permissionQueryWrapper.orderByAsc("pid");
        List<Permission> permissionsList = baseMapper.selectList(permissionQueryWrapper);
        for (Permission permission:permissionsList) {
            if("0".equals(permission.getPid())){
                // 找到后，给Permission中独有的两个属性，lever和children属性set值
                permission.setLevel(1);
                // bulidPermission2进行二三级分类
                permission.setChildren(bulidPermission2(permission,permissionsList));
    ...
    private List<Permission> bulidPermission2(Permission permission, List<Permission> permissionsList) {
        // 如果没有下一级，该属性赋默认值
        ...
        for (Permission per:permissionsList) {
            if(permission.getId().equals(per.getPid())){
                ...
                per.setChildren(bulidPermission2(per,permissionsList));
                permissions.add(per);
            }
        }
        return permissions;
    ...
    public void removeChildByIdGuli2(String id) {
        ...
        selectPermissionChildById2(id,permissions);
        baseMapper.deleteById(id);
    }

    private void selectPermissionChildById2(String id,List<Permission> permissions) {
        for (Permission permission:permissions) {
            if(id.equals(permission.getPid())){
                selectPermissionChildById2(permission.getId(),permissions);
